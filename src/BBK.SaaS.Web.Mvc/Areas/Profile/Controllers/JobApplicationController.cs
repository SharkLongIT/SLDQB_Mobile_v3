    ...
    public class JobApplicationController : SaaSControllerBase
    {
        ...
        public async Task<IActionResult> Detail(long Id)
        {
            ...
            if (jobApplicationModel.JobApplication.FileMgr != null)
            {
                ...
                {
                    jobApplicationModel.JobApplication.FileMgr.FileUrl = $"/file/get?c={StringCipher.Instance.Decrypt(jobApplicationModel.JobApplication.FileMgr.FilePath)}";
                }
            ...
            try
            {
                ...
                if (JobApplication.FileMgr != null)
                {
                    if (JobApplication.FileMgr.FilePath != null)
                    {
                        ...
                        JobApplication.FileMgr.FileName = StringCipher.Instance.Decrypt(JobApplication.FileMgr.FileName);
                        JobApplication.FileMgr.FilePath = StringCipher.Instance.Decrypt(JobApplication.FileMgr.FilePath);
                    }
                    int tenantId = AbpSession.TenantId ?? 1;
                    var fileMgr = new FileMgr() { TenantId = tenantId, FilePath = StringCipher.Instance.Decrypt(JobApplication.FileMgr.FilePath) };
                    //using (var profileImageService = await _fileServiceFactory.Get())
                    using (var profileImageService = _fileServiceFactory.Get())
                    {
                        JobApplication.FileMgr.FileToken = fileMgr.FileToken;
                    }
                }
        ...
        public async Task<JsonResult> CreateJobApp(JobApplicationEditDto JobApplicationEditDto)
        {
            try
            {
                var JobApplication = await _jobApplicationAppService.CreateJobApplication(JobApplicationEditDto);
                return Json(JobApplication);
            }
            catch (Exception ex)
            {
        ...
        public async Task<JsonResult> UpdateJobApplication(JobApplicationEditDto input)
        {
            try
            {
                var LearningProcess = await _jobApplicationAppService.UpdateJobApplication(input);
                return Json(LearningProcess);
            }
            catch (Exception ex)
            {
        ...
        //Model Dat Lich
        public async Task<IActionResult> MakeAnAppointment(long JobId)
        {
