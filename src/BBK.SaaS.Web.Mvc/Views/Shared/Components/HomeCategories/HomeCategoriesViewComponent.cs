...
public partial class HomeCategoriesViewComponent : SaaSViewComponent
{
    private readonly IArticlesAppService _articlesAppService;
    private readonly IFECntCategoryAppService _feCntCatService;

    public HomeCategoriesViewComponent(IArticlesAppService articlesAppService, IFECntCategoryAppService feCntCatService)
    {
    ...
    public async Task<IViewComponentResult> InvokeAsync(string zone, string type, long? categoryId)
    {
        ...
        var listArts = await _articlesAppService.GetFEArticles(new Mdls.Cms.Articles.MDto.SearchArticlesInput() { CategoryId = categoryId, MaxResultCount = 3 });
