			...
			},
			{
			  ...
			  render: function (data, type, row) {
				...
				if (row.contentType == 'application/pdf') {
				  reStr = `<i class="icofont icofont-file-pdf"> </i><a href="/file/get?c=` + row.publicUrl + `&ver=` + row.modified + `" target="_blank" />` + data + `</a>`;

				}
				else {
					reStr = `<img class="align-self-center img-fluid img-60 p-r-10" src="/file/get?c=` + row.thumbUrl + `&ver=` + row.modified + `" alt="#"><a href="/file/get?c=` + row.publicUrl + `&ver=` + row.modified + `" target="_blank" />` + data + `</a>`;
				}				
