...
namespace BBK.SaaS.Web.Areas.Profile.Controllers
{
    ...
    public class ApplicationRequestController : SaaSControllerBase
    {
        ...
        public async Task<IActionResult> ApplicationRequestByRecruiter()
        {
            #region kinh nghiệm
        ...
        //[HttpPost]
        public async Task<JsonResult> GetAll(ApplicationRequestSearchModel applicationRequestSearchModel)
        {
            var ApplicaitonRequest = await _appRequest.GetAll(applicationRequestSearchModel);
            return Json(ApplicaitonRequest);
        }

        public async Task<JsonResult> GetAllByRecruiter(ApplicationRequestSearchModel applicationRequestSearchModel)
        {
            var ApplicaitonRequest = await _appRequest.GetAllByRecruiter(applicationRequestSearchModel);
            return Json(ApplicaitonRequest);
        }


        public async Task<JsonResult> CreateUT(ApplicationRequestEditDto applicationRequest)
        {
            var AppRequest = await _appRequest.CreateUT(applicationRequest);
            return Json(AppRequest);
        }
