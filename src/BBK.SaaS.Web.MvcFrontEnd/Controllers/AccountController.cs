...
using BBK.SaaS.Authentication.TwoFactor.Google;
...
using BBK.SaaS.Authorization.Impersonation;
...
using BBK.SaaS.Web.Models.Account;
using BBK.SaaS.Web.Security.Recaptcha;
using BBK.SaaS.Web.Session;
using BBK.SaaS.Web.Views.Shared.Components.TenantChange;
...
namespace BBK.SaaS.Web.Controllers
{
	public class AccountController : SaaSControllerBase
	{
		...
		public AccountController(
				...
				IUserTypeAppService userTypeAppService, IAppConfigurationAccessor appConfigurationAccessor)
		{
				...
				string returnUrlHash = "", string ss = "")
		{
			...
			// Profile: Make sure only usertype can login
			if (loginResult.Result == AbpLoginResultType.Success)
			{
				if (_appConfiguration["App:IsFrontEndWeb"] != null && bool.Parse(_appConfiguration["App:IsFrontEndWeb"]))
				{
					if (loginResult.User.UserType == UserTypeEnum.Default)
					{
			...
			if (loginResult.User.ShouldChangePasswordOnNextLogin)
			{
				...
				var expireDate =
						Uri.EscapeDataString(Clock.Now.AddHours(1).ToString(SaaSConsts.DateTimeOffsetFormat));
		...
		public async Task<ActionResult> Register(RegisterViewModel model)
		{
			try
			{
				...
				{
					model.Surname = WebConsts.UnknowChar;
				}
				...
				if (_appConfiguration["App:IsFrontEndWeb"] != null && bool.Parse(_appConfiguration["App:IsFrontEndWeb"]))
				{
					if (model.UserType == UserTypeEnum.Default)
					{
						throw new UserFriendlyException("Bạn chưa lựa chọn thông tin người dùng");
					}
				}

				var user = await _userRegistrationManager.RegisterUserTypeAsync(
						model.Name,
						model.Surname,
						model.EmailAddress,
						model.UserName,
				...
				if (model.UserType != UserTypeEnum.Default)
				{
					await _userTypeAppService.Register(AbpSession.TenantId.Value, new Authorization.Users.Dto.UserEditDto()
					{
						Id = user.Id,
				...
				if (isEmailConfirmationRequiredForLogin)
					await SendEmailActivationLink(new SendEmailActivationLinkInput() { EmailAddress = user.EmailAddress });
		...
		[UnitOfWork(IsolationLevel.ReadUncommitted)]
		[HtmlSanitizer]
		public async Task<ActionResult> RegisterInternal(RegisterViewModel model)
		{
			try
			{
				if (!model.IsExternalLogin && UseCaptchaOnRegistration())
