(function () {
    ...
    var dataTable = _$RecruitmentTable.DataTable({
        ...
        pageLength: 10,
        "ajax": {
            "url": "/Profile/JobApplication/GetListJobAppOfCandidate",
            data: function (d) {
                d.search = $('#SearchTerm').val();
                if ($('#WorkSite').val() != 0 && $('#WorkSite').val() != "" && $('#WorkSite').val() != undefined && $('#WorkSite').val() != null) {
                    d.workSite = $('#WorkSite').val();
                }
                if ($('#FormOfWorkId').val() != 0 && $('#FormOfWorkId').val() != "" && $('#FormOfWorkId').val() != undefined && $('#FormOfWorkId').val() != null) {
                    d.formOfWorkId = $('#FormOfWorkId').val();
                }
                if ($('#OccupationId').val() != 0 && $('#OccupationId').val() != "" && $('#OccupationId').val() != undefined && $('#OccupationId').val() != null) {
                    d.occupationId = $('#OccupationId').val();
                }
                if ($('#LiteracyId').val() != 0 && $('#LiteracyId').val() != "" && $('#LiteracyId').val() != undefined && $('#LiteracyId').val() != null) {
                    d.literacyId = $('#LiteracyId').val();
                }
                if ($('#ExperiencesId').val() != 0 && $('#ExperiencesId').val() != "" && $('#ExperiencesId').val() != undefined && $('#ExperiencesId').val() != null) {
                    d.experiencesId = $('#ExperiencesId').val();
                }
            },
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    if (row.isPublished == false) {
                        return '<div class="form-check-size"><div class="form-check form-switch form-check-inline"><input class="form-check-input switch-primary check-size" id="checkPushlish" type="checkbox" role="switch" data-value="' + row.id + '"></div></div>'
                    }
                    else {
                        return '<div class="form-check-size"><div class="form-check form-switch form-check-inline"><input class="form-check-input switch-primary check-size" id="checkPushlish" type="checkbox" role = "switch"  data-value="' + row.id + '" checked = ""></div></div>'
                    }
                }
    ...
    $('#RecruitmentTable tbody').on('change', 'input[type="checkbox"]#checkDelete', function () {
        // If checkbox is not checked
        var selected = new Array();
        $('#RecruitmentTable tbody input[type="checkbox"]#checkDelete:checked').each(function () {
            selected.push($(this));
        });

        if (!this.checked) {
            selected = selected.filter(selected => selected !== $(this))
    ...
    //#region Xóa hồ sơ ứng tuyển
    function deleteJobApplication(data) {
        var record = data.record;
        abp.message.confirm(
            app.localize(abp.localization.localize("Xóa hồ sơ ứng tuyển " + data.record.title)),
            app.localize(abp.localization.localize("Bạn có chắc không?")),
            function (isConfirmed) {
                if (isConfirmed) {
                    var url = abp.appPath + "Profile/JobApplication/DeleteJobApplication";
                    var data = record;
                    callApi.ajax(url, data, getDocs, null, abp.notify.success(abp.localization.localize("Xóa thành công")))
                }
            }
        )
    ...
    //#region Update hồ sơ làm hồ sơ chính 

    $('#RecruitmentTable tbody').on('change', 'input[type="checkbox"]#checkPushlish', function () {
        var checkbox = $(this);
        ...
        if (checked === true) {
            abp.message.confirm(
                ...
                app.localize(abp.localization.localize("Bạn có chắc đặt làm hồ sơ chính không")),
                function (isConfirmed) {
                    if (isConfirmed) {
                        var url = abp.appPath + "Profile/JobApplication/UpdatePushlishById";
                        var data = {};
                        data.id = id;
                        callApi.ajax(url, data, getDocs, null, abp.notify.info(app.localize('Đặt làm hồ sơ chính thành công')))
            ...
            abp.message.confirm(
                app.localize(abp.localization.localize("")),
                app.localize(abp.localization.localize("Bạn có chắc chắn bỏ đặt làm hồ sơ chính không")),
                function (isConfirmed) {
                    if (isConfirmed) {
                        var url = abp.appPath + "Profile/JobApplication/UpdatePushlishById";
                        var data = {};
                        data.id = id;
                        callApi.ajax(url, data, getDocs, null, abp.notify.info(app.localize('Bỏ đặt làm hồ sơ chính thành công')))
