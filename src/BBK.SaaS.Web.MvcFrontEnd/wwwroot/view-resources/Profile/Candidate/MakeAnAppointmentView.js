(function () {
    ...
    var dataTable = _$AppointmentTable.DataTable({
        ...
        pageLength: 10,
        "ajax": {
            "url": "/Profile/Candidate/GetAllOfCandidate",
            data: function (d) {
                d.search = $('#SearchTerm').val();
                if ($('#StatusOfCandidate').val() != null && $('#StatusOfCandidate').val() != undefined && $('#StatusOfCandidate').val() != "") {
                    d.statusOfCandidate = $('#StatusOfCandidate').val();
                }
                d.rank = $('#Ranks').val();
        ...
        columnDefs: [
            ...
            },
            {
                ...
                data: "name",
                render: function (data, type, row, meta) {
                     return '<span> <a href=' + abp.appPath + "JobUser/NVNVViewDetailRecruiter/" + row.recruiter.userId + '>' + row.recruiter.companyName + '</a> </span>'
