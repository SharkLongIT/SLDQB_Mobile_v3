...
using BBK.SaaS.Test.Base.TestData;

namespace BBK.SaaS.Test.Base
{
    /// <summary>
    /// This is base class for all our test classes.
    /// It prepares ABP system, modules and a fake, in-memory database.
    /// Seeds database with initial data.
    /// Provides methods to easily work with <see cref="SaaSDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        protected TResult UsingDbContext<TResult>(Func<SaaSDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<SaaSDbContext, Task<TResult>> func)
        {
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<SaaSDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<SaaSDbContext, Task<TResult>> func)
        {
            TResult result;
